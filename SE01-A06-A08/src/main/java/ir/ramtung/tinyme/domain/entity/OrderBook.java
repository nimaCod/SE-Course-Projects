...
public class OrderBook {
    ...
    public OrderBook(LinkedList<Order> buyQueue, LinkedList<Order> sellQueue) {
    ...
    public void update(Order order) {
        ...
        ListIterator<Order> it = queue.listIterator();
        while (it.hasNext()) {
            if (order.getOrderId() == it.next().getOrderId()) {
                it.previous();
                it.next().quantity = order.getQuantity();
                // it.remove();
                break;
            }
        }
    ...
    public void pushAtEnd(Order order){
